-- Migration to update user registration trigger to set BUSINESS_ID
...
-- Create updated function that sets BUSINESS_ID
...
BEGIN
    ...
    IF ACCOUNT_TYPE = 'admin' THEN
        ...
        -- Update the profile to mark as partner admin and set primary partner and business_id
        UPDATE PROFILES
        SET
            IS_PARTNER_ADMIN = TRUE,
            PRIMARY_PARTNER_ID = NEW_PARTNER_ID,
            BUSINESS_ID = NEW_PARTNER_ID,
            UPDATED_AT = NOW(
            )
        WHERE
            ID = NEW.ID;
    ...
    ELSIF ACCOUNT_TYPE = 'partner' THEN
        ...
        -- Update profile to mark as NOT partner admin and set business_id to NULL
        UPDATE PROFILES
        SET
...
COMMENT ON FUNCTION HANDLE_NEW_USER_REGISTRATION() IS
    'Handles new user registration: admin users become partner admins (business owners), partner users become system admins (platform admins). Sets BUSINESS_ID for multi-tenant access.';