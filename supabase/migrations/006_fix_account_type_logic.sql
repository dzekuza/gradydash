...
CREATE OR REPLACE FUNCTION HANDLE_NEW_USER_REGISTRATION() RETURNS TRIGGER AS $$
DECLARE 
    NEW_PARTNER_ID UUID;
    COMPANY_NAME TEXT;
    ACCOUNT_TYPE TEXT;
BEGIN
    ...
    ELSIF ACCOUNT_TYPE = 'partner' THEN
        ...
        INSERT INTO PARTNERS (
            ...
            COMPANY_NAME,
            'store-' || NEW.ID::TEXT,
            'Store created by ' || COALESCE(NEW.RAW_USER_META_DATA->>'full_name', NEW.EMAIL),
            NEW.ID
...
COMMENT ON FUNCTION HANDLE_NEW_USER_REGISTRATION() IS 'Handles new user registration based on account type: admin users become system admins, partner users become partner admins';
