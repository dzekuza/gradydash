...
CREATE TYPE ROLE AS
  ...
  CREATE TYPE PRODUCT_STATUS AS
    ...
    CREATE TABLE MEMBERSHIPS ( ID UUID DEFAULT UUID_GENERATE_V4() PRIMARY KEY, ENVIRONMENT_ID UUID REFERENCES ENVIRONMENTS(ID) ON DELETE CASCADE, USER_ID UUID REFERENCES PROFILES(ID) ON DELETE CASCADE, ROLE ROLE NOT NULL DEFAULT 'reseller_staff', CREATED_AT TIMESTAMP WITH TIME ZONE DEFAULT NOW(), UPDATED_AT TIMESTAMP WITH TIME ZONE DEFAULT NOW(), UNIQUE(ENVIRONMENT_ID, USER_ID) );
 
    -- Create locations table
    CREATE TABLE LOCATIONS ( ID UUID DEFAULT UUID_GENERATE_V4() PRIMARY KEY, ENVIRONMENT_ID UUID REFERENCES ENVIRONMENTS(ID) ON DELETE CASCADE, NAME TEXT NOT NULL, DESCRIPTION TEXT, ADDRESS TEXT, CREATED_AT TIMESTAMP WITH TIME ZONE DEFAULT NOW(), UPDATED_AT TIMESTAMP WITH TIME ZONE DEFAULT NOW() );
    ...
    CREATE TABLE PRODUCT_IMAGES ( ID UUID DEFAULT UUID_GENERATE_V4() PRIMARY KEY, PRODUCT_ID UUID REFERENCES PRODUCTS(ID) ON DELETE CASCADE, STORAGE_PATH TEXT NOT NULL, FILE_NAME TEXT NOT NULL, FILE_SIZE INTEGER NOT NULL, MIME_TYPE TEXT NOT NULL, UPLOADED_BY UUID REFERENCES PROFILES(ID) ON DELETE SET NULL, CREATED_AT TIMESTAMP WITH TIME ZONE DEFAULT NOW() );
 
    -- Create sales table
    CREATE TABLE SALES ( ID UUID DEFAULT UUID_GENERATE_V4() PRIMARY KEY, PRODUCT_ID UUID REFERENCES PRODUCTS(ID) ON DELETE CASCADE, AMOUNT DECIMAL(10, 2) NOT NULL, CURRENCY TEXT NOT NULL DEFAULT 'EUR', SOLD_BY UUID REFERENCES PROFILES(ID) ON DELETE SET NULL, SOLD_AT TIMESTAMP WITH TIME ZONE DEFAULT NOW(), CREATED_AT TIMESTAMP WITH TIME ZONE DEFAULT NOW() );
 
    -- Create environment_invites table
    CREATE TABLE ENVIRONMENT_INVITES ( ID UUID DEFAULT UUID_GENERATE_V4() PRIMARY KEY, ENVIRONMENT_ID UUID REFERENCES ENVIRONMENTS(ID) ON DELETE CASCADE, EMAIL TEXT NOT NULL, ROLE ROLE NOT NULL, INVITED_BY UUID REFERENCES PROFILES(ID) ON DELETE SET NULL, ACCEPTED_AT TIMESTAMP WITH TIME ZONE, EXPIRES_AT TIMESTAMP WITH TIME ZONE NOT NULL, CREATED_AT TIMESTAMP WITH TIME ZONE DEFAULT NOW() );
    ...
    -- Staff and admins can modify locations
    ...
    -- Members can upload images in their environments
    CREATE POLICY "Members can create product images" ON PRODUCT_IMAGES FOR INSERT WITH CHECK (
    ...
    CREATE POLICY "Users can delete their own images" ON PRODUCT_IMAGES FOR
    DELETE USING ( UPLOADED_BY = AUTH.UID() );
 
    -- Staff and admins can delete any image
