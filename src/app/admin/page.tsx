...
import { Building2, Users, BarChart3, Settings, MapPin } from 'lucide-react'
...
export default async function AdminDashboardPage() {
  ...
  const [partnersCount, usersCount, locationsCount] = await Promise.all([
    supabase.from('partners').select('id', { count: 'exact', head: true }),
    supabase.from('profiles').select('id', { count: 'exact', head: true }),
    supabase.from('admin_locations').select('id', { count: 'exact', head: true })
  ])
